...
export default function HeroCalculator({
  ...
  return (
    <div className={`${hasAmount ? "bg-[#00c853]" : "bg-white"} rounded-[20px] p-6 sm:p-8 relative overflow-hidden transition-colors duration-300`}>
      {/* Emoji background — only when result showing */}
      {hasAmount && (
      ...
      <div className="relative">
        {!hasAmount ? (
          <>
            <p className="text-[11px] font-semibold uppercase tracking-[1px] text-[#888] mb-3">How much are you saving?</p>
            <div className="grid grid-cols-4 gap-2">
              {AMOUNT_BRACKETS.map((a) => (
                <button
                  key={a.value}
                  onClick={() => onAmountChange(a.value)}
                  className="py-3 rounded-xl text-[13px] font-bold transition-all bg-[#f5f5f5] text-[#1a1a1a] hover:bg-[#e8e8e8]">
                  {a.label}
                </button>
              ))}
            </div>
