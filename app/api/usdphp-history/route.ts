...
const [dailyData, setDailyData] = useState<[number, number][]>([]); // Frankfurter daily, back to 1999
const [intradayData, setIntradayData] = useState<[number, number][]>([]); // CoinGecko hourly, last 30d
...
// Daily history from Frankfurter via API route (1999–today, Vercel caches 3hr)
useEffect(() => {
...
const data = await res.json();
if (data?.prices?.length) setDailyData(data.prices as [number, number][]);
} catch { /* no daily data */ }
}
loadDaily();
}, []);

// Intraday from CoinGecko client-side (30 days = hourly, small request from browser)
useEffect(() => {
async function loadIntraday() {
try {
const res = await fetch("https://api.coingecko.com/api/v3/coins/tether/market_chart?vs_currency=php&days=30");
if (!res.ok) throw new Error(res.statusText);
const data = await res.json();
if (data?.prices?.length) setIntradayData(data.prices as [number, number][]);
} catch { /* no intraday data */ }
}
loadIntraday();
}, []);

// Pick source + slice based on period
const chartData = useMemo(() => {
// 1D/1W → intraday (hourly). 1M/1Y/ALL → daily.
const useIntraday = period === "1" || period === "7";
const source = useIntraday ? intradayData : dailyData;
...
const sliced = source.filter(d => d[0] >= cutoff);
pts = sliced.length >= 2 ? sliced : source.slice(-2);
}
const maxPoints = 150;
